...
def get_action(
    ...
    *,
    ...
    if need_log_prob:
        ...
        action = _forward_action(policy, states, deterministic)
...
def _forward_action(policy: nn.Module, states: torch.Tensor, deterministic: bool) -> torch.Tensor:
    ...
    if hasattr(policy, "sample_actions"):
        return policy.sample_actions(states, K=1, seed=None)[:, 0, :]
    out = policy(states)
    return out[0] if isinstance(out, (tuple, list)) else out
