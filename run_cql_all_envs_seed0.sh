#!/bin/bash
# CQL 환경별 seed 0 순차 실행
# D4RL MuJoCo: halfcheetah-medium ~ walker2d-medium-replay
...
SEED=0
BASE_CONFIG="configs/offline/cql_pogo_base.yaml"

echo "=========================================="
echo "CQL 전체 환경 순차 학습 (seed $SEED)"
echo "expert: w2_weights [100, 100], 나머지: [10, 10]"
echo "=========================================="

for env in "${ENVIRONMENTS[@]}"; do
    env_clean=$(echo "$env" | sed 's/-/_/g')
    log_dir="logs/cql/${env_clean}/seed${SEED}"
    mkdir -p "$log_dir"
    log_file="${log_dir}/$(date +%Y%m%d_%H%M%S).log"

    TEMP_CONFIG="/tmp/cql_pogo_${env_clean}_seed${SEED}.yaml"
