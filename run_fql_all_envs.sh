#!/bin/bash
# POGO Multi-Actor FQL 모든 환경 훈련 스크립트 (JAX)
...
echo "=========================================="
echo "POGO Multi-Actor FQL 훈련 시작"
echo "총 ${#ENVIRONMENTS[@]}개 환경"
echo "=========================================="
echo ""

LOG_DIR="logs/pogo_fql"
mkdir -p "$LOG_DIR"
...
for env in "${ENVIRONMENTS[@]}"; do
    ...
    # 예: halfcheetah-medium-v2 -> configs/offline/pogo_multi/halfcheetah/medium_v2_fql.yaml
    env_dir=$(echo "$env" | cut -d'-' -f1)
    env_task=$(echo "$env" | cut -d'-' -f2- | sed 's/-/_/g')
    config_file="configs/offline/pogo_multi/${env_dir}/${env_task}_fql.yaml"
    ...
    TEMP_CONFIG="/tmp/pogo_fql_${env_clean}_seed${SEED}.yaml"
...
echo "=========================================="
echo "모든 POGO Multi-Actor FQL 훈련 완료!"
echo "=========================================="
