#!/bin/bash
# POGO Multi-Actor SAC-N 모든 환경 훈련 스크립트
...
# POGO base config 사용
BASE_CONFIG="configs/offline/sac_n_pogo_base.yaml"
...
echo "=========================================="
echo "POGO Multi-Actor SAC-N 훈련 시작"
echo "총 ${#ENVIRONMENTS[@]}개 환경"
echo "=========================================="
echo ""

LOG_DIR="logs/pogo_sac_n"
mkdir -p "$LOG_DIR"
...
for env in "${ENVIRONMENTS[@]}"; do
    ...
    # 예: halfcheetah-medium-v2 -> configs/offline/pogo_multi/halfcheetah/medium_v2_sac_n.yaml
    env_dir=$(echo "$env" | cut -d'-' -f1)
    env_task=$(echo "$env" | cut -d'-' -f2- | sed 's/-/_/g')
    config_file="configs/offline/pogo_multi/${env_dir}/${env_task}_sac_n.yaml"
    ...
    TEMP_CONFIG="/tmp/pogo_sac_n_${env_clean}_seed${SEED}.yaml"
...
echo "=========================================="
echo "모든 POGO Multi-Actor SAC-N 훈련 완료!"
echo "=========================================="
