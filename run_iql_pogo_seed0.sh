...
# w2_weights: expert는 [100.0, 100.0], 나머지는 [10.0, 10.0] (command line에서 설정)
...
echo "=========================================="
echo "IQL 전체 환경 순차 학습 (Gaussian Policy)"
...
for env in "${ENVIRONMENTS[@]}"; do
    ...
    log_dir="logs/iql/${env_name_clean}/seed${SEED}"
    mkdir -p "$log_dir"
    ...
    if [[ "$env" == *"expert"* ]]; then
        W2_WEIGHTS_ARG="'[100.0, 100.0]'"
        echo "    [expert] w2_weights: [100.0, 100.0]"
    else
        W2_WEIGHTS_ARG="'[10.0, 10.0]'"
    fi
