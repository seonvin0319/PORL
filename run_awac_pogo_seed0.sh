#!/bin/bash
# AWAC 전체 환경 순차 실행 (seed 0)
# awac_pogo_base.yaml 사용, actor_configs: stochastic 설정됨
...
ALGORITHM="awac"
SEED=0
CONFIG_PATH="configs/offline/awac_pogo_base.yaml"

echo "=========================================="
echo "AWAC 전체 환경 순차 학습 (Stochastic Policy)"
...
for env in "${ENVIRONMENTS[@]}"; do
    ...
    log_dir="logs/awac/${env_name_clean}/seed${SEED}"
    mkdir -p "$log_dir"
    ...
    # 임시 config 파일 생성
    TEMP_CONFIG="/tmp/awac_pogo_${env//-/_}_seed${SEED}.yaml"
