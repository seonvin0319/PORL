...
def build_mlp_layers(
    ...
    activation: type = F.relu,
):
    """공통 MLP 레이어 빌딩 함수 (Policy 및 Critic용)
    
    Args:
        input_dim: 입력 차원
        hidden_dim: Hidden layer 차원 (기본값: 256)
        n_hiddens: Hidden layer 개수 (기본값: 2)
        output_dim: 출력 차원 (None이면 head 없이 base layers만 반환)
        layernorm: LayerNorm 사용 여부
        activation: 활성화 함수 (기본값: F.relu)
    ...
    # output_dim이 없으면 base layers만 반환 (policy용)
    if output_dim is None:
        return layers, current_dim
    
    # output_dim이 있으면 완전한 Sequential용 레이어 리스트 반환 (critic용)
    result_layers = []
    for i, layer in enumerate(layers):
        result_layers.append(layer)
        result_layers.append(activation() if callable(activation) else activation)
        if layernorm:
...
class BasePolicyMLP(nn.Module):
    """Base class for policy MLPs with common forward pattern"""
    def __init__(self, state_dim: int, action_dim: int, max_action: float, 
...
class GaussianMLP(BasePolicyMLP):
    ...
    def __init__(self, state_dim, action_dim, max_action, hidden_dim: int = 256, n_hiddens: int = 2, tanh_mean: bool = True):
    ...
    def __call__(self, states, *args, **kwargs):
        ...
        if len(args) == 0 and len(kwargs) == 0:
            ...
            # AWAC 방식: unbounded mean에서 샘플링 후 clamp
            if not self.tanh_mean:
                # AWAC 방식: policy.rsample() 후 clamp
...
class TanhGaussianMLP(BasePolicyMLP):
    ...
    def __init__(self, state_dim, action_dim, max_action, hidden_dim: int = 256, n_hiddens: int = 2,
        ...
        self.separate_mu_logstd = separate_mu_logstd  # SAC-N/EDAC용: mu와 log_std를 별도 레이어로
        self.is_gaussian = False  # TanhGaussian은 closed form W2 사용 불가
        self.is_stochastic = True
        
        if separate_mu_logstd:
            # SAC-N/EDAC 방식: mu와 log_std를 별도 레이어로 출력
            self.mu = nn.Linear(self.head_input_dim, action_dim)
            self.log_std_head = nn.Linear(self.head_input_dim, action_dim)
        else:
            # 기본 방식: mean만 출력, log_std는 공유 파라미터
            self.head = nn.Linear(self.head_input_dim, action_dim)
            self.log_std = nn.Parameter(torch.zeros(action_dim, dtype=torch.float32))
    ...
    def _get_mean_logstd(self, states):
        ...
        if self.separate_mu_logstd:
            ...
            mean = self.head(x)
            log_std = self.log_std.clamp(self.log_std_min, self.log_std_max)
            # log_std를 배치 차원으로 확장: [action_dim] -> [B, action_dim]
            if log_std.dim() == 1:
    ...
    def log_prob(self, observations: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        """CQL과 호환: log_prob 계산
        간단한 구현: 실제 log_prob 대신 0 반환 (CQL에서는 실제로 사용하지 않음)
        """
        B = observations.size(0)
        return torch.zeros(B, device=observations.device)
    ...
    def __call__(self, states, *args, repeat=None, deterministic=False, need_log_prob=False, **kwargs):
        ...
        if repeat is not None:
            ...
            # log_prob는 간단히 0으로 반환 (CQL에서는 실제로 사용하지 않음)
            log_probs = torch.zeros(B * repeat, device=actions.device)
