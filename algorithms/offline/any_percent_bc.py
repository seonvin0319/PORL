...
import wandb
from .networks import DeterministicMLP
...
class Actor(DeterministicMLP):
    """any_percent_bc용 Actor wrapper - forward만 사용 (action만 반환)"""
    def __init__(self, state_dim: int, action_dim: int, max_action: float):
        super().__init__(state_dim, action_dim, max_action, hidden_dim=256, n_hiddens=2)
    
    def __call__(self, state: torch.Tensor) -> torch.Tensor:
        """any_percent_bc는 action만 필요하므로 forward만 호출"""
