...
SEED=1
BASE_CONFIG="configs/offline/awac_pogo_base.yaml"

echo "=========================================="
echo "AWAC 전체 환경 순차 학습 (seed $SEED)"
echo "expert: w2_weights [100, 100], 나머지: [10, 10]"
echo "=========================================="

for env in "${ENVIRONMENTS[@]}"; do
    env_clean=$(echo "$env" | sed 's/-/_/g')
    log_dir="logs/awac/${env_clean}/seed${SEED}"
    mkdir -p "$log_dir"
    log_file="${log_dir}/$(date +%Y%m%d_%H%M%S).log"

    TEMP_CONFIG="/tmp/awac_pogo_${env_clean}_seed${SEED}.yaml"
